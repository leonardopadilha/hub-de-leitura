...
db.serialize(() => {
    ...
    if (books.length > 0) {
        ...
        books.forEach((book, index) => {
            try {
                ...
                if (!uniqueIsbn || duplicatesFound.includes(uniqueIsbn)) {
                    uniqueIsbn = `978-85-AUTO-${String(index + 1).padStart(4, '0')}-${Math.floor(Math.random() * 10)}`;
                    console.log(`⚠️  ISBN gerado automaticamente para "${book.title}": ${uniqueIsbn}`);
                }
                ...
                console.error(`❌ Erro ao inserir livro "${book.title}":`, error.message);
            }
        });
        ...
        if (duplicatesFound.length !== books.length) {
            console.log(`⚠️  ${books.length - booksInserted} livros tiveram problemas na inserção`);
        }
    } else {
    ...
    users.forEach((userData, index) => {
        bcrypt.hash(userData.password, saltRounds, (err, hashedPassword) => {
            if (err) {
                console.error(`❌ Erro ao hashear senha para ${userData.name}:`, err.message);
                return;
            }

            db.run(
                "INSERT INTO Users (name, email, password, phone, isAdmin) VALUES (?, ?, ?, ?, ?)",
                [userData.name, userData.email, hashedPassword, userData.phone, userData.isAdmin],
                (err) => {
                    if (err) {
                        console.error(`❌ Erro ao inserir usuário ${userData.name}:`, err.message);
                    } else {
    ...
    function insertTestData() {
        ...
        const testReservations = [
            {
                ...
                status: 'active',
                pickup_deadline: new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString() // 48h a partir de agora
            },
            {
                user_id: 2, // João Silva
                book_id: 3, // O Pequeno Príncipe
                status: 'completed',
                reservation_date: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000).toISOString(), // 10 dias atrás
                pickup_date: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000).toISOString(), // 8 dias atrás
                return_date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString() // 1 dia atrás
            }
        ];
        ...
        db.run("UPDATE Books SET available_copies = available_copies - 1 WHERE id = 1");

        console.log('✅ Dados de teste inseridos');
        console.log('\n🎉 Banco de dados da biblioteca criado com sucesso!');
        console.log('\n📋 Resumo:');
        console.log('   • Usuários: Admin (admin@biblioteca.com) e Teste (joao@teste.com)');
